...
class doubleCluster(DefaultChart):
    ...
    def plot(self, **kwargs): 
        ...
        for c in ax.containers:
            ax.bar_label(c, fmt='{:,.1f}M'.format, labels=[ round(val / self.scale, 2) for val in c.datavalues], fontsize=self.bar_label_size, label_type='center')
        ...
        for category in stacked_pivot.columns:
            ...
            ax.bar_label(p, fmt='{:,.1f}M'.format, labels=[ round(val / self.scale, 2) for val in p.datavalues], fontsize=self.bar_label_size, label_type='center')
